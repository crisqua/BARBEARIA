...
5. Upload real de logo — `POST /v1/tenants/me/logo`, **MVP local em disco** (`apps/api/uploads/`, servido estático por `main.ts`). ~~Continua pendente antes de produção: trocar por Supabase Storage/presigned URL (seção 6.1.6) — Render não tem disco persistente entre deploys.~~ Resolvido, ver revisão 2026-09-23.
...
8. Login em tenant **suspenso** agora dá `403` com mensagem específica ("Esta barbearia está com o acesso suspenso...") em vez de cair no `401` genérico de credenciais inválidas — decisão consciente, confirmada com o usuário: revela a um tentante de login que aquele tenant existe e está suspenso (trade-off aceito deliberadamente), mas melhora a experiência do dono de uma barbearia suspensa de verdade, que agora sabe o motivo real. Tenant inexistente e senha errada continuam com `401` genérico (`auth.service.ts:validateCredentials`) — só a suspensão ficou específica.

**Risco operacional identificado, registrado de propósito (não é decisão, é um problema real observado 2x já):** a suíte `test:e2e` não usa um banco separado — `DATABASE_URL`/`DIRECT_URL` do `.env` local apontam pro mesmo Postgres usado pra homologação manual. Cada rodada da suíte cria dezenas de tenants de fixture (`login-test-*`, `provision-*`, `dup-*`, `suspend-*`, `pay-admin-*` etc.) que sujam a lista de barbearias do Painel Master — já precisou de limpeza manual duas vezes durante o piloto. Correção óbvia (banco de teste separado, `.env.test` com `DATABASE_URL` própria) ainda **não implementada** — usuário optou por só registrar por enquanto, não corrigir agora. Antes de rodar `npm run test:e2e` de novo em ambiente de homologação, ter isso em mente.
...
**Revisão (2026-09-23):** upload de logo trocado de disco local pra **presigned URL do Supabase Storage** (seção 6.1.6), resolvendo a pendência registrada na revisão 2026-08-22 — item real que impedia produção de verdade (Render não persiste disco entre deploys). Fluxo em 3 passos: `POST /v1/tenants/me/logo/presign` (novo, `tenants.controller.ts`) valida o `contentType` e devolve uma URL assinada via `TenantLogoStorageService`; o painel-barbearia sobe o arquivo direto pro Supabase com `@supabase/supabase-js` (`uploadToSignedUrl`), sem passar pelo NestJS; confirma a URL final reaproveitando o `PATCH /v1/tenants/me` que já existia (`logoUrl` já era campo aceito no DTO). Tamanho (2MB) e tipos aceitos (`png`/`jpeg`/`svg`) são garantidos pela config do bucket `tenant-logos` no Supabase, não só por validação em código. `useStaticAssets('/uploads')` removido de `main.ts` — não serve mais nada. Variáveis novas: `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` (backend, privilegiada) e `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY` (frontend, pública por design — autorização real do upload vem do token assinado, não dessa chave). Client Supabase inicializado sob demanda dos dois lados (não no boot) — enquanto o bucket/variáveis não existirem em um ambiente, só o upload de logo falha ali, não a aplicação inteira.
